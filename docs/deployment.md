...
Pylon runs independently, not inside the serial customer sync. It runs at minute 27 every three hours, leaving one retry window before its six-hour freshness deadline. PostHog retains its existing 100-user customer-sync pass and 20-user hourly pass. For a support-only refresh, use `bun run --filter=agent support:sync`.
